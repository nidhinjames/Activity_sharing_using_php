...
{
	this.sublevelarrow={src:'../libcommon/images/admin_images/images/right_b.gif', width:'8px', top:'3px', left:'6px'} //full URL to image used to indicate there's a sub level
	this.breadcrumbarrow='../libcommon/images/admin_images//images/right_b.gif' //full URL to image separating breadcrumb links (if it's defined)
	this.loadingimage='../libcommon/images/admin_images/images/loader.gif' //full URL to 'loading' image, if menu is loaded via Ajax
	...
	this.backarrow='../libcommon/images/admin_images/images/leftarrow.gif' //full URL to image added in front of back LI 
	...
	{
		if (thisdrill.filesetting.url && thisdrill.filesetting.url.length>0)
		{ //menu defined inside external file (use Ajax)?
			var $dest=(typeof thisdrill.filesetting.targetElement=="string")? $('#'+thisdrill.filesetting.targetElement) : null
			if (!$dest || $dest.length!=1)
			{ //if target element isn't defined or multiple targets found
...
drilldownmenu.prototype.buildmenu=function($){
	...
	this.$uls.each(function(i){ //loop through each UL
		...
		if (i==0){ //if topmost UL
			//$('<li class="backcontroltitle">Menu</li>').prependTo($thisul).click(function(e){e.preventDefault()})
			...
			//alert($thisul.outerHeight());
		   thisdrill.$maindiv.css({height:'600px', overflow:'hidden'})//main div's height manipulated by francis must change when items added to main menu
			...
			$('<li id="backbutton"><button class="backcontrol">Back</button></li>') //add back LI item
		...
		//alert($thisul.outerHeight());
		var ulheight=($thisul.outerHeight() < thisdrill.$maindiv.data('h'))? $thisul.outerHeight() : 'auto'
		//var ulheight=($thisul.outerHeight() < thisdrill.$maindiv.data('h'))? thisdrill.$maindiv.data('h') : 'auto'
		$thisul.css({visibility:'visible', width:'100%', height:ulheight, left:(i==0)? 'auto' : $parentul.outerWidth(), top:0, float:'right'})
...
drilldownmenu.prototype.updatenav=function($, endorder)
{
	...
	{ 
		//if breadcrumb div defined
		var $crumb=this.navdivs.$crumb.empty()	
		if (endorder>0)
		{ 
			//if this isn't the topmost UL (no point in showing crumbs if it is)
			var crumbhtml=''
			while ($currentul && $currentul.data('specs').order>=0)
			{
				crumbhtml='<a href="#nav" rel="'+$currentul.data('specs').order+'">'+$currentul.data('specs').title+'</a><div class="breadcrumb_divider"></div>'+crumbhtml
				$currentul=($currentul.data('specs').order>0)? this.$uls.eq($currentul.data('specs').parentorder) : null
			}			
			$crumb.append(crumbhtml).find('.breadcrumb_divider').last().remove()		
			$crumb.find('img:eq(0)').remove().end().find('a:last').css({'color':'#000'})	
		}
		else
		{
			$crumb.append('<span>'+this.homecrumbtext+'</span>')
		}
	}
